...
function doGet(e) {
  try {
    ...
    if (e && e.parameter && e.parameter.action === 'ping') {
      return json_({ ok: true, message: 'Ket noi Apps Script thanh cong', spreadsheetUrl: ss.getUrl(), sheets: ss.getSheets().map(function(s) { return s.getName(); }) });
    }
    return json_(readAll_(ss));
  } catch (err) {
    return json_({ ok: false, error: String(err && err.message || err) });
  }
}
